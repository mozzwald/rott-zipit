...
//    sdl_surface = SDL_SetVideoMode (320, 200, 8, flags);
   flags = SDL_FULLSCREEN | SDL_SWSURFACE | SDL_HWPALETTE;
    sdl_surface_real = SDL_SetVideoMode (320, 240, 16, flags);
    sdl_surface_16b_small = SDL_CreateRGBSurface(flags,320,200, 16, 0, 0, 0, 0);
    sdl_surface = SDL_CreateRGBSurface(flags, 320, 200, 8, 0, 0, 0, 0);
	if (sdl_surface_real == NULL)
	{
		Error ("Could not set video mode\n");
	}
	if (sdl_surface == NULL)
	{
		Error ("Could not create 8bit surface\n");
	} 
}
...
void SetTextMode ( void )
{
	if (SDL_WasInit(SDL_INIT_VIDEO) == SDL_INIT_VIDEO) {
		if (sdl_surface != NULL) {
			...
			SDL_FreeSurface(sdl_surface_real);
...
static void Stretch240 ()
{
  ...
  dest.w = 320;
  dest.h = 240;
  SDL_SoftStretch(sdl_surface_16b_small, &src, sdl_surface_real, &dest);
}
...
if (DoStretch) {
   SDL_BlitSurface (sdl_surface, NULL, sdl_surface_16b_small, NULL);
   Stretch240();
   SDL_UpdateRect (sdl_surface_real, 0, 0, 320, 240);
} else {
   SDL_Rect offset; offset.x=0; offset.y=20;
   SDL_FillRect(sdl_surface_real, NULL, SDL_MapRGB(sdl_surface_real->format, 0, 0, 0));
   SDL_BlitSurface (sdl_surface, NULL, sdl_surface_real, &offset);
   SDL_UpdateRect (sdl_surface_real, 0, 0, 320, 240);
}
}
